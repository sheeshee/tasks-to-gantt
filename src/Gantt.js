...
export const Gantt = ({ tasks: { data } }) => {
  const tasks = data.map(task => ({ ...task, id: task['Task ID'], startDate: task.start_date }))
  const columns = [
  ...
  return (
      <MaterialTheme ><BaseGantt tasks={tasks} columns={columns} /></MaterialTheme>
  )
}

Gantt.propTypes = {
  tasks: Task.isRequired
}
